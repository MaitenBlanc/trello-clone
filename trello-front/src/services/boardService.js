...
export const boardService = {
    ...
    createList: async (boardId, listName) => {
        const response = await api.post(`/boards/${boardId}/lists`, {name: listName});
        return response.data;
    },

    createCard: async (boardId, listId, cardTitle) => {
        const response = await api.post(`/boards/${boardId}/lists/${listId}/cards`, {title: cardTitle});
        return response.data;
    },

    deleteCard: async (boardId, listId, cardId) => {
        const response = await api.delete(`/boards/${boardId}/lists/${listId}/cards/${cardId}`);
        return response.data;
    },
